document.addEventListener('DOMContentLoaded', () => {
  // --- Countdown timer (for student exam page) ---
  const timeEl = document.getElementById('time');
  // initial remaining seconds provided by template (or 0)
  const initialRemaining = window.__INIT_TIME_REMAINING__ || 0;
  ...
  if (timeEl && initialRemaining > 0) {
    startCountdown(initialRemaining);
  }

  // --- Question builder (teacher UI) ---
  ...
  // Only wire up builder when present on the page
